...
public class SFPolygon : MonoBehaviour
{
	...
	public int pathCount
	{
		...
		set
		{
			...
			if (value < 1)
			{
				UnityEngine.Debug.LogError("pathCount must be positive.");
				return;
			}
			if (value == 1)
			{
	...
	public void CopyFromCollider(Collider2D collider)
	{
		...
		{
			UnityEngine.Debug.LogWarning("CopyFromCollider() only works with polygon and box colliders.");
		}
	}
	...
	public void _CopyFromCollider()
	{
		...
		UnityEngine.Debug.LogWarning("GameObject has no polygon or box collider. Adding default SFPolygon shape instead.");
