...
public class IKLegMove : MonoBehaviour
{
	...
	private void DoSteps()
	{
		...
		{
			Vector3 a = transformGroundPos - currentTransformGroundPos;
			a *= Mathf.Clamp(footCurveUp.Evaluate(animationTime) * 1f, 0f, 1f);
			previousTransformGroundPos += a;
			currentTransformGroundPos += a;
		}
	}
	...
	private void MoveFoot()
	{
		...
		else if ((bool)currentHitTransform)
		{
			...
			{
				a2 = Vector3.Lerp(a, vector, animationTime / animationLength);
				d = footCurveUp.Evaluate(animationTime);
			}
			target.position = a2 + Vector3.up * d;
		}
	}
