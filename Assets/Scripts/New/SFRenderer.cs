...
public class SFRenderer : MonoBehaviour
{
	...
	private void Start()
	{
		...
		UnityEngine.Debug.Log("SFSS init: " + graphicsDeviceType + ", " + (UV_STARTS_AT_TOP ? "UV_STARTS_AT_TOP, " : "") + lightmapFormat + ", " + lightmapFormatHDR);
	}
	...
	private void RenderLightMap(Matrix4x4 viewMatrix, Matrix4x4 projection, Matrix4x4 vpMatrix, RenderTexture target, List<SFLight> lights, List<SFPolygon> polys, Color ambient, bool hdr)
	{
		...
		for (int i = 0; i < lights.Count; i++)
		{
			...
			Matrix4x4 rhs = sFLight._ModelMatrix(forceProjection: false) * sFLight._CookieMatrix();
			Rect rect = ScissorRect(vpMatrix * rhs, width, height);
			Matrix4x4 matrix4x = ClipMatrix(rect, 2f / (float)width, 2f / (float)height) * projection;
			GL.Viewport(rect);
			...
			GL.LoadProjectionMatrix(TextureProjectionMatrix(matrix4x * viewMatrix * rhs));
	...
	{
		Color ambientLight = _ambientLight;
		ambientLight.a = 1f;
		Matrix4x4 lhs = Matrix4x4.Ortho(_extents.xMin, _extents.xMax, _extents.yMin, _extents.yMax, 1f, -1f);
		...
		RenderLightMap(worldToCameraMatrix, matrix4x, vpMatrix, _lightMap, _culledLights, null, ambientLight, allowHDR);
		if (_shadows)
		{
			...
			RenderLightMap(worldToCameraMatrix, matrix4x, vpMatrix, _ShadowMap, _culledLights, _culledPolygons, ambientLight, allowHDR);
	...
	private void OnPostRender()
	{
		if (_fogColor.a + _scatterColor.r + _scatterColor.g + _scatterColor.b > 0f)
		{
			...
			Color scatterColor = _scatterColor;
			scatterColor.a = _softHardMix;
