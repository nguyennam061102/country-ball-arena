...
public class RayCastTrail : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < array.Length; i++)
        {
            ...
            //Player component = array[i].transform.root.GetComponent<Player>(); //todo: old
//             BaseCharacter component = array[i].transform.GetComponent<BaseCharacter>();
//             
//             if ((!component || !(timeAtSpawn + t >= Time.time)) && (!ignoredRig || !(ignoredRig == array[i].rigidbody)))
//             {
//                 ProjectileHitSurface component2 = array[i].collider.GetComponent<ProjectileHitSurface>();
//                 if ((!component2 || component2.HitSurface(HitInfo.GetHitInfo(array[i]), base.gameObject) != 0) &&
//                     array[i].distance < raycastHit2D.distance)
//                 {
//                     raycastHit2D = array[i];
//                 }
//             }
            //if (!(timeAtSpawn + t >= Time.time) && array[i].distance < raycastHit2D.distance)
            {
                raycastHit2D = array[i];
            }
        }
