...
public class ProjectileCollision : ProjectileHitSurface
{
	...
	{
		Debug.Log("Projectile Collision Hit Surface");
	...
	public void Die()
	{
		...
		if (!hasCollided)
		{
			...
			if (sparkObject != null)
			{
				Instantiate(sparkObject, base.transform.position, base.transform.rotation).transform.localScale = Vector3.one * ((startDMG / 55f + 1f) * 0.5f);
			}
