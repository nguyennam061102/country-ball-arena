	...
	public class IKMapping
	{
		...
		public class BoneMap
		{
			...
			public void UpdatePlane(bool rotation, bool position)
			{
				...
				if (rotation)
				{
					defaultLocalTargetRotation = QuaTools.RotationToLocalSpace(transform.rotation, lastAnimatedTargetRotation);
				}
				if (position)
				{
					planePosition = Quaternion.Inverse(lastAnimatedTargetRotation) * (transform.position - planeBone1.position);
				}
			}
