...
namespace RootMotion.FinalIK
{
	public class GenericPoser : Poser
	{
		...
		{
			if (!(weight <= 0f) && (!(base.localPositionWeight <= 0f) || !(base.localRotationWeight <= 0f)) && !(poseRoot == null))
			{
				float localRotationWeight = base.localRotationWeight * weight;
				float localPositionWeight = base.localPositionWeight * weight;
				for (int i = 0; i < maps.Length; i++)
				{
					maps[i].Update(localRotationWeight, localPositionWeight);
				}
			}
		}
