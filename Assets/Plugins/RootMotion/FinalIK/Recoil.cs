...
namespace RootMotion.FinalIK
{
	public class Recoil : OffsetModifier
	{
		...
		protected override void OnModifyOffset()
		{
			...
			Quaternion rhs = (aimIK != null && aimIK.solver.transform != null && !aimIKSolvedLast) ? Quaternion.LookRotation(aimIK.solver.IKPosition - aimIK.solver.transform.position, ik.references.root.up) : ik.references.root.rotation;
			rhs = randomRotation * rhs;
			RecoilOffset[] array = offsets;
			for (int i = 0; i < array.Length; i++)
			{
				array[i].Apply(ik.solver, rhs, w, length, endTime - Time.time);
			}
			if (!handRotationsSet)
			{
			...
			if (twoHanded)
			{
				...
				Vector3 a = primaryHand.position + primaryHandEffector.positionOffset + handRotation * point;
				secondaryHandEffector.positionOffset += a - (secondaryHand.position + secondaryHandEffector.positionOffset);
			}
