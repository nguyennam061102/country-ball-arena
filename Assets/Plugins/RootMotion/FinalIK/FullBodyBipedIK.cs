...
namespace RootMotion.FinalIK
{
	...
	public class FullBodyBipedIK : IK
	{
		...
		public bool ReferencesWarning(ref string warningMessage)
		{
			...
			if (Vector3.Dot(rhs: (solver.rootNode.position - references.leftUpperArm.position).normalized, lhs: vector.normalized) > 0.95f)
			{
			...
			if (Vector3.Dot(rhs: (solver.rootNode.position - references.leftThigh.position).normalized, lhs: vector2.normalized) > 0.95f)
			{
